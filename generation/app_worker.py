...
def generate_application(plan: ReadPlan, behavior: "Behavior | None" = None,
    ...
    return _SCAFFOLD.format(
        ...
        threshold=(behavior.threshold if behavior is not None else 0),
...
def _emit_body(plan: ReadPlan, p: str, behavior: "Behavior | None" = None,
    ...
    if (last_raw {cmp_} APP_THRESHOLD_RAW) {{
        gpio_write(1);
...
/* config.h carries the tunable values when this file is built as part of the
 * generated repo. Built standalone (no include path) the defaults below apply,
 * so the source still compiles on its own. */
...
#endif
#ifndef APP_THRESHOLD_RAW
#define APP_THRESHOLD_RAW {threshold}u
#endif
...
# Canonical build-system targets this generator can produce.
# Any value from the spec that does not appear here is an error — never a
# silent fallback to a different build system (that is the original bug).
SUPPORTED_OUTPUT_TARGETS = ("make", "cmake")
...
# -lgcc: -nostdlib drops libgcc's soft-float helpers, which the driver needs if
# it converts anything to float.
LDLIBS   = -lgcc
...
# CMake toolchain file for bare-metal Cortex-M4 cross-compilation.
# Kept separate from CMakeLists.txt so the project file stays readable and
# the toolchain can be re-used by any target in the tree.
_CMAKE_TOOLCHAIN = """# Generated by EmbeddPilot V2 — bare-metal Cortex-M4 cross-compilation.
# Pass to cmake with: cmake -DCMAKE_TOOLCHAIN_FILE=cmake/arm-none-eabi.cmake ..
set(CMAKE_SYSTEM_NAME Generic)
set(CMAKE_SYSTEM_PROCESSOR arm)

# Prefer EMBEDDPILOT_ARM_GCC env var (the same override the pipeline uses),
# then fall back to whatever is on PATH.
if(DEFINED ENV{EMBEDDPILOT_ARM_GCC})
    set(CROSS_PREFIX $ENV{EMBEDDPILOT_ARM_GCC})
    # The env var points at the gcc binary itself; strip the "gcc" suffix so we
    # can derive the other tools from the same prefix.
    string(REGEX REPLACE "gcc$" "" CROSS_PREFIX "${CROSS_PREFIX}")
else()
...
set(CMAKE_C_FLAGS_INIT   "${CPU_FLAGS} -Os -ffreestanding -nostdlib -nostartfiles -Wall -Wextra")
set(CMAKE_EXE_LINKER_FLAGS_INIT "${CPU_FLAGS} -nostdlib -nostartfiles")
"""

_CMAKELISTS = """# Generated by EmbeddPilot V2.
cmake_minimum_required(VERSION 3.16)
project(firmware C)

# -lgcc: -nostdlib drops libgcc's soft-float helpers, which the driver needs if
# it converts anything to float (e.g. __aeabi_i2f).  Link it back explicitly.
# CMAKE_EXE_LINKER_FLAGS is append-only here so the toolchain's base flags are
# preserved.
set(CMAKE_EXE_LINKER_FLAGS "${CMAKE_EXE_LINKER_FLAGS} -T ${CMAKE_SOURCE_DIR}/link/stm32f4.ld -lgcc")

add_executable(firmware.elf src/main.c)
target_include_directories(firmware.elf PRIVATE include)

# Produce a raw binary alongside the ELF.
...
def _readme(plan: ReadPlan, behavior, verified: dict,
            output_target: str = "make") -> str:
    steps = "\n".join(
        f"- `{s.label}` — {s.kind} at "
    ...
    # Build instructions vary by the build system that was actually emitted.
    # If the repo build was not verified (tool absent), say so plainly — the
    # README must not claim the repo builds when that was never tested.
    repo_build_status = verified.get("repo_build", None)
    if output_target == "cmake":
        build_cmd = ("    mkdir build && cd build\n"
                     "    cmake -DCMAKE_TOOLCHAIN_FILE=../cmake/arm-none-eabi.cmake ..\n"
                     "    cmake --build .")
        build_note = "Requires `cmake` (3.16+) and `arm-none-eabi-gcc`."
    else:  # make (default)
        build_cmd = "    make            # -> build/firmware.elf"
        build_note = "Requires `arm-none-eabi-gcc` and `make`."

    if repo_build_status == "skipped":
        skip_reason = verified.get("repo_build_skip_reason", "build tool not available")
        repo_build_note = (
            f"\n> **Repo build NOT verified** — {skip_reason}. "
            f"The pipeline confirmed the source compiles, but building through "
            f"the repo's own build system was skipped. Verify manually before "
...
| Repo builds through its own build system | {verified.get('repo_build', 'not checked')} |
...
def generate_project(
    ...
    output_target: str = "make",
) -> dict[str, str]:
    """The deliverable: a complete, buildable application repo.

    `output_target` must be one of SUPPORTED_OUTPUT_TARGETS ("make", "cmake").
    Passing an unrecognised value raises AppGenerationError — never a silent
    fallback to a different build system.
    ...
    """
    if output_target not in SUPPORTED_OUTPUT_TARGETS:
        raise AppGenerationError(
            f"output_target {output_target!r} is not supported. "
            f"Supported targets: {', '.join(SUPPORTED_OUTPUT_TARGETS)}. "
            f"Refusing to substitute a different build system."
        )
    main_c = generate_application(plan, behavior, samples, has_math_oracle)
    _verified = verified or {}
    files: dict[str, str] = {
        "src/main.c": main_c,
        # Every value a user might reasonably want to change, in ONE place with
        # its provenance. Without this the repo is readable but not adjustable:
        # the bus address and the trigger threshold are buried in the middle of
        # generated C, and changing them means editing code you did not write.
        "include/config.h": _config_h(plan, behavior, samples),
        "README.md": _readme(plan, behavior, _verified, output_target=output_target),
    }
    if output_target == "cmake":
        files["CMakeLists.txt"] = _CMAKELISTS
        files["cmake/arm-none-eabi.cmake"] = _CMAKE_TOOLCHAIN
    else:  # make
...
def _config_h(plan: ReadPlan, behavior, samples: int) -> str:
    """The configuration surface, separated from the code that uses it.

    Each entry says WHERE it came from. A value taken from the datasheet-derived
    register map is not the same kind of thing as one the user chose, and a
    header that blurs the two invites someone to 'fix' a register address."""
    p = _ident(plan.chip)
    regs = "\n".join(
        f"/* {s.label:<24} from the extracted register map */\n"
        f"#define {p}_REG_{s.label.split('-')[-1].upper():<18} 0x{s.reg:02X}u"
        for s in plan.steps if s.reg is not None)
    beh = ""
    if behavior is not None:
        beh = f"""
/* ---- the rule you asked for ------------------------------------------- */
/* Compared against the RAW device word: converting it to engineering units
 * needs a datasheet-grounded conversion, and where none exists this project
 * refuses to invent one. Change the threshold, not the units. */
#define APP_THRESHOLD_RAW    {behavior.threshold}u
#define APP_ACTUATOR_PIN     {behavior.gpio_pin}u   /* GPIO{behavior.gpio_port} */
"""
    return f"""/* Configuration for the {plan.chip} application, generated by EmbeddPilot.
 *
 * Everything here is safe to change EXCEPT the register offsets: those came
 * from the datasheet-extracted map and were cross-checked against it. Editing
 * one makes the firmware disagree with the device it was verified against.
 */
#ifndef EMBEDDPILOT_CONFIG_H
#define EMBEDDPILOT_CONFIG_H

/* ---- the device ------------------------------------------------------- */
/* bus address — as you stated it in the requirement */
#define {p}_ADDR             0x{plan.address:02X}u

/* register offsets — from the extracted register map, NOT chosen by you */
{regs}

/* ---- how often ------------------------------------------------------- */
#define APP_SAMPLES          {max(1, samples)}u
{beh}
#endif /* EMBEDDPILOT_CONFIG_H */
"""
...
def generate_system(
    ...
    if behavior is not None:
        ...
        if (sys_raw {cmp_} APP_THRESHOLD_RAW) {{
            gpio_write(1);
            uart_puts("{behavior.action_label}=ON\\r\\n");
        }} else {{
            gpio_write(0);
